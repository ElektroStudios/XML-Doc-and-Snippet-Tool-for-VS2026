...
- Automates the insertion of common XML documentation tags directly from the code editor context menu or via keyboard shortcuts:
  ...
  - Wraps selected text in formatting tags: `<b>`, `<i>`, `<u>`, `<para>`, `<c>`, `<code>`, `<remarks>`.
  - Wraps selected code blocks in fully structured `<example><code language="...">` sections.
  - Inserts XML documentation separator lines with a configurable fill character and length.
- Automates `.snippet` file generation from selected code, pre-filling author metadata from the extension settings.
- Collapses, expands, and deletes all XML documentation comment blocks in the active document in one operation.
- Each command can be individually enabled or disabled from **Tools → Options → Snippet Tool**.
- Compatible with C# and VB.NET source files.
